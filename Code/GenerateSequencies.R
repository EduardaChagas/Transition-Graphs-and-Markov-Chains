...
parameters.estimators <- function(ts, dimension = 3, delay = 1){
...
parameters.estimators.by.patterns <- function(ts, next.pattern, dimension = 3, delay = 1){
  N = length(ts)
  new.ts = rep(0, N + 1)
  new.ts[1:N] = ts
  
  metrics.elements = parameters.estimators(ts)
  
  new.ts[N + 1] = rnorm(1, mean = metrics.elements[next.pattern, 1], sd = metrics.elements[next.pattern, 2])
  return(new.ts)
}

# Generation of elements of the patterns samples -------------------------------

generate.sequence <- function(ts, n.new.elements, dimension = 3, delay = 1){
  new_ts = ts
  ...
  for(i in 1:n.new.elements){
    ...
    new_ts = parameters.estimators.by.patterns(new_ts, as.integer(next.pattern))
  }
  return(new_ts)
}

generate.sequence.watg <- function(ts, n.new.elements, dimension = 3, delay = 1){
  new_ts = ts
